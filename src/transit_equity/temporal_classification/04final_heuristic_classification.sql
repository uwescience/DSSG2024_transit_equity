...
BEGIN
    RETURN CASE
        WHEN EXTRACT(hour FROM device_dtm) BETWEEN 5 AND 10 THEN 'morning'
        WHEN EXTRACT(hour FROM device_dtm) BETWEEN 11 AND 14 THEN 'noon'
...
--the home location is within the afternoon earliest trips.
*/
with days_with_both_trips AS (
...
--the home location is at the earliest noon locations.
*/
with days_with_both_trips AS (
    SELECT base_query.card_id AS card_id, base_query.date AS date 
        FROM base_query 
...
--The home location is at the pre-dawn/morning -- the earliest boarding in the morning is probably near home
*/
...
/*       
--A: > 6 days per month, B: > 1 day per month      
--8A&B (frequent/moderate_long_afternoon_commuter) average afternoon and per-dawn days > threshold above (will this differ much from category 7?)      
some uncertainty on whether the predawn or the afternoon trip is home location.
•  A few further rules to check:
1. night shifters—- afternoon location is home location
	• If one of the top two locations are close to warehouse, hospitals,airports, manufactoring, more likely to be night shift worker
	• If weekly commute days<= 4, likely night shift workers
2. If the top 2 locations, one has a lower rent than the other one, then the lower rent place is the home location
very early commuters -then the predawn location is home location*/
...
/*       
--A: > 6 days per month, B: > 1 day per month 
--9A&B (frequent/moderate_noontime_activity) average noon and afternoon days > threshold above
--the noon locations contain home
*/
with days_with_both_trips AS (
    SELECT base_query.card_id AS card_id, base_query.date AS date
        FROM base_query 
...
--the most frequent or second most frequent location might be home. 
--If frequencies are very close, decide on the cluster with lower rent will likely be home
*/
...
--The first trip on weekend will contain home locations
*/
...
/*---G12 people who made two trips a day but both trips are within same major time of day, 
multiple days
---the first boardings in a day with most frequency will likely be home locations.
*/
with days_with_both_trips AS (
    SELECT base_query.card_id AS card_id, base_query.date AS date
        FROM base_query 
...
IF distinct days/ month >=1 but not fall into any previous categories
* If morning trips account for more than 1/3 of trips, 
then use the first boarding in the morning for home location
* else if noon trips account for more than 1/3 of trips, 
then use the first boarding in the noon for home location
* else find the typical earliest trip time for boarding and use that for home location
*/
